...
describe('<Table>', () => {
  ...
  it('skal vise korrekt antall rader og kolonner', () => {
    ...
    const wrapper = shallow(
      <Table headerTextCodes={headerTextCodes}>
        <TableRow key={1} id="1" onMouseDownRow={callbackFunction} onKeyDownRow={callbackFunction}>
          <TableColumn key={1}>{12345}</TableColumn>
          <TableColumn key={2}>{23446}</TableColumn>
        </TableRow>
        <TableRow key={2} id="2" onMouseDownRow={callbackFunction} onKeyDownRow={callbackFunction}>
