...
describe('behandlingsprosessKlageSelectors', () => {
  it('skal lage behandlingspunkter for engangsstønad', () => {
    ...
    const behandlingType = bt.FORSTEGANGSSOKNAD;
    const vilkar = [];
    ...
    const behandlingsresultat = { type: 'test' };

    const props = getBehandlingspunkterProps.resultFunc(fagsakYtelseType, behandlingType, vilkar, aksjonspunkter, behandlingsresultat);

    expect(props).has.length(2);
    expect(props[0].code).is.eql(behandlingspunktCodes.FORMKRAV_KLAGE_NAV_FAMILIE_OG_PENSJON);
    expect(props[1].code).is.eql(behandlingspunktCodes.KLAGE_RESULTAT);
  });
