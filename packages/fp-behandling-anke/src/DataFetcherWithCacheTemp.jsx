...
import { AnkeBehandlingApiKeys } from './data/ankeBehandlingApi';
...
const mapping = {
  [AnkeBehandlingApiKeys.ANKE_VURDERING]: 'anke-vurdering',
};
...
const mapStateToPropsFactory = () => {
  const createProps = createSelector([(state, ownProps) => ownProps.data[0].getRestApiData()(state), (state, ownProps) => ownProps.data], (
    ...
    behandlingData: data
      .reduce((acc, d) => ({
        ...acc,
        [format(d.name)]: d.name === AnkeBehandlingApiKeys.BEHANDLING ? behandling : behandling[mapping[d.name]],
      }), {}),
  }));
