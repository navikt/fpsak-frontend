...
describe('<YtelsePanel>', () => {
  ...
  it('skal vise ytelsepanel for sykepenger', () => {
    ...
    expect(formattedMessages).to.have.length(5);

    expect(formattedMessages.get(1).props.values.ytelse).to.equal('Sykepenger');
    expect(formattedMessages.get(1).props.id).to.equal('BeregningInfoPanel.TilstøtendeYtelseForm.Ytelse');

    expect(formattedMessages.get(3).props.values.dekningsgrad).to.equal(100);
    expect(formattedMessages.get(3).props.id).to.equal('BeregningInfoPanel.TilstøtendeYtelseForm.Sykepengerdekning');
  });
});
