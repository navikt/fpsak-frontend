...
import { InnsynBehandlingApiKeys } from './data/innsynBehandlingApi';
...
const mapping = {
  [InnsynBehandlingApiKeys.INNSYN]: 'innsyn',
};
...
const mapStateToPropsFactory = () => {
  const createProps = createSelector([(state, ownProps) => ownProps.data[0].getRestApiData()(state), (state, ownProps) => ownProps.data], (
    ...
    behandlingData: data
      .reduce((acc, d) => ({
        ...acc,
        [format(d.name)]: d.name === InnsynBehandlingApiKeys.BEHANDLING ? behandling : behandling[mapping[d.name]],
      }), {}),
  }));
