...
envsubst '$DEBUG $APP_DIR $APP_PATH_PREFIX $APP_API_GATEWAY $APP_API_PATH $OIDC_AGENTNAME $OIDC_PASSWORD $OIDC_HOST_URL $RESOLVER $APP_NAME $APP_VERSION $FASIT_ENVIRONMENT_NAME $APP_PORT $APP_CALLBACK_PATH $APP_LOGIN_PATH $SESSION_STORAGE $APP_HOSTNAME' < /etc/nginx/conf.d/app.conf.template > /etc/nginx/conf.d/default.conf
