...
docker build . -t docker.adeo.no:5000/fpsak-frontend-test:${RELEASE_VERSION} -f ./docker/DockerfileDev --build-arg HTTP_PROXY=${HTTP_PROXY} --build-arg HTTPS_PROXY=${HTTP_PROXY}
